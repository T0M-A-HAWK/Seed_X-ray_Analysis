...
from PyQt5.QtWidgets import QAction, QApplication, QCheckBox, QLabel, QMainWindow, QStatusBar, QToolBar, QScrollArea
...
qtCreatorFile = "C:\Temp_Data\PyQt-GUI\Seed-xray-analysis-X.ui" # Enter file here.
...
class MyApp(QtWidgets.QMainWindow, Ui_MainWindow):
        ...
        #presets menu setup
            with open('AcaciaData.csv', 'r') as obj:
                ...
                for row in csv_reader:
                    self.presetValues.append(row)                    
            menuPresets = self.menubar_file.addMenu("&Presets")
            menuPresets.triggered.connect(self.actionClicked)
            
            for i in range(len(self.presetValues) - 1):
                menuPresets.addAction(str('&' + str(self.presetValues[i + 1][0])))
                
        def actionClicked(self, action):
            #print(action.text())
            for j in range(int(len(self.presetValues) - 1)):
                if str(action.text()) == str('&' + str(self.presetValues[j + 1][0])):
                    self.rowset = j + 1
                    self.SetPreset()
        ...
        def mousePressEvent(self, event):
            ...
            def markersChange():
                if temp.x() < event.x() < (temp.width() + temp.x()) and temp.y() + self.menubar_file.height() < event.y() < (temp.height() + temp.y() + self.menubar_file.height()):
                    ...
                    scroll = QScrollArea(self.image_view)
                    scroll.verticalScrollBar().setValue(scroll.verticalScrollBar().value() + 90)
    ...
    #        self.image = np.uint8(-(self.DICOMimage -np.max(self.DICOMimage ))*(255/(np.max(self.DICOMimage )-np.min(self.DICOMimage ))))
        ...
        def SetPreset(self):
            self.species = str(self.presetValues[self.rowset][0])
            if int(self.presetValues[self.rowset][2]) > 0:
                for i in range(int(self.presetValues[self.rowset][2])):
                    self.IncreaseContrast()
            if int(self.presetValues[self.rowset][2]) < 0:
                for i in range(abs(int(self.presetValues[self.rowset][2]))):
                    self.ReduceContrast()    
            self.image_resolution_spinBox.setValue(int(self.presetValues[self.rowset][3]))      
            self.UpdateDataResolution()
            self.small_objects_spinBox.setValue(int(self.presetValues[self.rowset][4]))
            self.large_objects_spinBox.setValue(int(self.presetValues[self.rowset][5]))
            self.CleanupImage()
            self.background_spinBox.setValue(int(self.presetValues[self.rowset][6]))
            self.foreground_spinBox.setValue(float(self.presetValues[self.rowset][7]))
            self.DefineSearchArea()
            self.DetectSeeds()
            self.CalcRegionProps()
            self.min_intensity_spinBox.setValue(int(self.presetValues[self.rowset][8]))
            self.max_intensity_spinBox.setValue(int(self.presetValues[self.rowset][9]))
            self.UpdateIntensity()
            self.viability_spinBox.setValue(int(self.presetValues[self.rowset][10]))
            self.ClassifyViable()
